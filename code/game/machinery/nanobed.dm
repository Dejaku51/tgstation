...
/obj/machinery/nanobed/update_icon()
	. = ..()
	if(nanites_running())
		. += "nanobed_on"
...
/obj/machinery/nanobed/post_buckle_mob(mob/living/L)
	...
	if(nanites_running())
		link_nanites(L)
		SEND_SIGNAL(L, COMSIG_NANITE_SIGNAL, trigger, "nanobed")
	update_icon()
...
/obj/machinery/nanobed/ui_interact(mob/user, datum/tgui/ui)
	if( is_operational() )
		ui = SStgui.try_update_ui(user, src, ui)
		if(!ui)
			ui = new(user, src, "Nanobed")
			ui.open()
...
/obj/machinery/nanobed/ui_data()
	...
	data["current_nanites"] = internal_nanites.nanite_volume
	data["max_nanites"] = internal_nanites.max_nanites
